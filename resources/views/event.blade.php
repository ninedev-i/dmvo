...
if ( $event->post_reliz && Input::get('show') != 'anons' ) {
  if (file_exists("img/events/covers/event_id".$event->id.".jpg")) {echo "<a href='".URL::to('public/img/events/covers')."/event_id".$event->id.".jpg' data-fancybox><img src='".URL::to('public/img/events/covers')."/event_id".$event->id.".jpg' style='width: 250px; float: right; margin: 0 15px 15px 15px;' alt='".$event->title."' title='".$event->title."'></a>";}
  if ($event->content) {echo "<p><b>Пост-релиз</b> <a href='".URL::to('events/'.$event->id.'?show=anons')."' style='margin-left: 10px;'>Анонс</a></p>";}
